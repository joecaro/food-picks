...
export function ScoreVotingForm({ foodFightId, restaurants, userScores, endTime }: ScoreVotingFormProps) {
    const [scores, setScores] = useState<ScoresState>({});
    // We will adjust the hook later to handle single score submissions and optimistic updates
    // For now, keep track of which specific score is being submitted
    const [submittingScore, setSubmittingScore] = useState<string | null>(null); // Format: "restaurantId-score"
    const { mutate: submitScore, error, isPending } = useSubmitScores(String(foodFightId)); // Renamed mutate for clarity
    ...
    const isVotingEnded = useMemo(() => new Date() > new Date(endTime), [endTime]);

    return (
        <div className="space-y-6">
            <h2 className="text-2xl font-semibold mb-4">Vote for Restaurants</h2>
            {isVotingEnded && (
                 <p className="text-red-500 font-medium">Voting has ended for this Food Fight.</p>
            )}
            <div className="space-y-4">
                {restaurants.map((restaurant) => (
                    <div key={restaurant.id} className="p-4 border rounded-lg shadow-sm bg-card">
                        ...
                        <div className="flex space-x-2 items-center justify-center">
                            {Object.entries(scoreEmojis).map(([scoreValue, emoji]) => {
                                ...
                                return (
                                    <button
                                        type="button"
                                        key={scoreValue}
                                        onClick={() => !isVotingEnded && !isSubmittingThis && handleScoreChange(restaurant.id, scoreNum)}
                                        disabled={isVotingEnded || isSubmittingThis || (isPending && !isSubmittingThis)} // Disable buttons while *any* score is submitting, or specifically this one
                                        className={`text-3xl p-2 rounded-full transition-transform transform hover:scale-110 ${
                                            isSelected ? 'bg-primary/20 scale-110' : 'opacity-70 hover:opacity-100'
                                        } ${
                                            isVotingEnded || isSubmittingThis ? 'cursor-not-allowed opacity-50' : ''
                                        } ${
                         ...
                         {/* Use restaurant.id (string) directly as key */}
                         {scores[restaurant.id] && (
                             <p className="text-center mt-2 text-sm text-muted-foreground">Your score: {scores[restaurant.id]}</p>
                         )}
                         {/* Display general error, could be more specific */}
                         {error && (
                            <p className="text-red-500 text-center mt-2 text-xs">Error updating score. Please try again.</p>
                         )}
                    </div>
                ))}
            </div>
